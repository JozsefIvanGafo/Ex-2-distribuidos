...
int set_value(int key, char *value1, int N_value2, double *V_value2, struct Coord value3) {
    // Validar parámetros
    if (key < 0 || N_value2 <= 0 || N_value2 > 32 || value1 == NULL || V_value2 == NULL) {
        return -1;
    }
    ...
    if (sendMessage(sock, buffer, strlen(buffer)) < 0) {
        close(sock);
        return -1;
    }
...
int get_value(int key, char *value1, int *N_value2, double *V_value2, struct Coord *value3) {
    if (key < 0 || value1 == NULL || N_value2 == NULL || V_value2 == NULL || value3 == NULL) {
        return -1;
    }
    ...
    if (token == NULL || strcmp(token, "OK") != 0) return -1;
